...
- become uni_adm
- ssh to stage server
- inside your preferred path, create a folder named taxonomy "mkdir taxonomy"
- inside just created taxonomy folder, create a new folder named "git-repository"
- inside just created git-repository folder, execute the command "git clone https://user:password@scm.ebi.ac.uk/git/unp
.fw.taxonomy.git". (You should use Jenkins "uni_scm" user if it is a new VM Machine)
- inside taxonomy folder copy deployment bin folder to it "cp -rf git-repository/unp.fw.taxonomy/taxonomy-deployment/src/main/resources/* ."
- execute command "chmod 755 *.sh", "cd bin", "chmod 755 *.*"
- make sure your environment.properties and config.properties are correct
- inside bin folder execute the command "./deploy-taxonomy.sh"
- done